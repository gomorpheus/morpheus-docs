...
One of the first things |morpheus| does when it is tied into a vRO integration is sync all available workflows by category. These workflows become available when creating a new |morpheus| task in :menuselection:`Library --> Automation`. |morpheus| allows a user to map these vRO workflows into the task engine. The task engine allows users to design workflows that chain tasks in order or operate at different phases of a provisioning request. For more information on tasks, please read the Automation documentation.

Creating a task for vRO is simple.

First, go to :menuselection:`Library --> Automation` and create a new task.  Enter a Name and a Code, the Code can be used later to reference the results of tasks.  Choose a task type of ``vRealize Orchestrator Workflow``. A dropdown will appear allowing one to first select the active vRO Integration you would like to use. Once that is selected, a list of workflows becomes available.
