...
During provisioning, users do not need to create or select a keypair as you would do when provisioning directly from the AWS console. If needed, |morpheus| will create an AWS keypair without input from the user. In the AWS console and in the |morpheus| Keypairs Section (:menuselection:`Infrastructure --> Keys & Certs --> Key Pairs`) you will see these keypairs listed. |morpheus| will also inventory other keypairs which are created in your integrated AWS clouds and these will be listed out in the |morpheus| Keypairs section.
