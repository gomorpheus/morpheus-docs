...
AI Agents define the behavior of the "Ask Murph" assistant. Each agent ties together a language model, a system prompt, and a set of tool providers (MCP servers). Multiple agents can be configured per account, with one designated as the default.
...
.. list-table::
   ...
   * - **Default**
     - Mark as the default agent for the account. Only one agent can be default. The default agent is used when users open Ask Murph without selecting a specific agent.
   * - **MCP Servers**
...
- A user opens Ask Murph without specifying an agent
