.. _Release Notes 9.0.0:

**************************
v9.0.0 Feature Release Notes
**************************
...
:Platform Updates: - Embedded MySQL updated to 8.4.8; external MySQL configuration includes the 8.4 LTS family, subject to release-specific certification of the external version and service variant
...
:HVM: - Fixed an issue where virtual machines on a data network with a 9000 MTU could not communicate using jumbo packets until the network interface was manually reset
      ...
      - Automatic MII polling-rate, up-delay, and down-delay defaults for HVM Active/Passive bonds are not guaranteed in 9.0 because the original provisioning change was reverted. Inspect the effective bond configuration and configure required values explicitly through tooling supported by the installed HVM release; values can vary by release and networking backend
