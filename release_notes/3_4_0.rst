...
.. IMPORTANT:: Morpheus v3.4.0 adds support for subtenant users to login via the main tenant url using subtenant name or id prefix, ie subtentantName\username or subtentantId\username. Subtenant local users will no longer be able to login from main login url without using subtenant name or id prefix.
