...
Appliance: Nginx now defaults to ``tls 1.2`` only
   The default Nginx config removes support for tls 1.0 and 1.1.
    ...
    - |morpheus| front-end Load Balancer monitors/checks that do not support TLS 1.2 will by default not succeed. Updating checks to TLS 1.2 support or configuring nginx to allow lower tls versions via morpheus.rb config will be required.
