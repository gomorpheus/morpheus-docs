...
- 5.4.5: Warning: Database indexes added for account_usage and metadata_tag tables. Customers with very large account_usage and/or metadata_tag tables (10 million+) may experience slower initial morphues-ui loading time after upgrading to 5.4.5, as well as additional database load. 
...
- 5.4.5: A10 Load Balancer type has been diabled, and will no longer be an option when adding new Load Balancers. Contact |morpheus| if you need to re-enable A10 Load Balancer option. This does not affect existing Load Balancers.
