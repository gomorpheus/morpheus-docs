...
on:
  ...
  steps:
    - name: Skip localization bot loop commits
      ...
      run: |
        if echo "$COMMIT_MSG" | grep -qiE '^\[localize/de\]|^chore\(localize/de\):'; then
...
safe-outputs:
  create-pull-request:
    title-prefix: "[localize/de] "
    labels: [localization, de, automation]
    draft: true
    branch-prefix: localize/de/
...
# Incremental German (de) localization

Synchronize German Sphinx gettext translations when English documentation changes.
...
4. Create a **draft pull request** with:
   - Title prefix already set to `[localize/de]`
   - Body listing:
...
Do not modify files outside `locale/de/`. Do not touch `tools/translate/work/`.

Commit message format: `chore(localize/de): sync translations for English doc changes`
