...
on:
  ...
  steps:
    - name: Skip localization bot loop commits
      ...
      run: |
        if echo "$COMMIT_MSG" | grep -qiE '^\[localize/pt\]|^chore\(localize/pt\):'; then
...
safe-outputs:
  create-pull-request:
    title-prefix: "[localize/pt] "
    labels: [localization, pt, automation]
    draft: true
    branch-prefix: localize/pt/
...
# Incremental Portuguese (pt) localization

Synchronize Portuguese Sphinx gettext translations when English documentation changes.
...
4. Create a **draft pull request** with:
   - Title prefix already set to `[localize/pt]`
   - Body listing:
...
Do not modify files outside `locale/pt/`. Do not touch `tools/translate/work/`.

Commit message format: `chore(localize/pt): sync translations for English doc changes`
