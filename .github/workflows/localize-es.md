...
on:
  ...
  steps:
    - name: Skip localization bot loop commits
      ...
      run: |
        if echo "$COMMIT_MSG" | grep -qiE '^\[localize/es\]|^chore\(localize/es\):'; then
...
safe-outputs:
  create-pull-request:
    title-prefix: "[localize/es] "
    labels: [localization, es, automation]
    draft: true
    branch-prefix: localize/es/
...
# Incremental Spanish (es) localization

Synchronize Spanish Sphinx gettext translations when English documentation changes.
...
4. Create a **draft pull request** with:
   - Title prefix already set to `[localize/es]`
   - Body listing:
...
Do not modify files outside `locale/es/`. Do not touch `tools/translate/work/`.

Commit message format: `chore(localize/es): sync translations for English doc changes`
