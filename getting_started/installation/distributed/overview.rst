...
3-Node HA (Recommended)
  In the current 3-Node HA architecture, the Application, Messaging, and Non-Transactional Database tiers are deployed across the three |morpheus| application nodes. The Transactional Database Tier is not installed on those nodes. Configure all three application nodes to use the same external MySQL cluster or supported MySQL service. This external database is a required part of the 3-Node HA topology, not an optional co-located tier.
