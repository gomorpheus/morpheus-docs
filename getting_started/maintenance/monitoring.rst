...
|morpheus| provides status and data for all critical system services, including morpheus-ui, mysql, elasticsearch and rabbitmq services, in the Health section/endpoint. The |morpheus| system **Health** page is accessible via the ui at ``/administration/heatlh``, the API at ``/api/health`` and the CLI with ``morpheus health``. External monitoring tools can utilize the morpheus api to check status and data values for system and morphues-ui memory and cpu data as well as local and external services, pending sufficinet permissions for external services. 
...
- ``/var``
  ...
  The default locations for critical service data paths are located in /var, including morphues-ui, elasticsearch, mysql and rabbitmq data. Elasticsearch and RabbitMQ have default disk space thresholds that can cause service disruption when reached.

  - Warning: <80%
  - Critical <95% 
  
    When ``/var`` has <95% free space available (or 95% or greater used), Elasticsearch will by default enfore read-only (cluster.routing.allocation.disk.watermark.flood_stage) preventing any writes as well as causing rabbitmq messages to become backlogged, eventually leading to memory issues and possible morpheus-ui shutdown.
...
.. important:: 80% of system memory by default will always be allocated to java heap. If elasticsaearch is local then 35% of total system memory is allocated to morphues-ui heap and 45% to elasticsearch heap. If elasticsearch is external, then 80% of total system memory is allocated to morpheus-ui heap. This means basic memory stats will always show system memory utilization at or above 80% regardless of actual memory usage. 

morpheus-ui memory
^^^^^^^^^^^^^^^^^^

Various services and tools provide jvm statics such as jvmtop and jstat. The |morpheus| api also provides morphues-ui memory utilizaiton in addition to overall system memory data. 

The |morpheus| api ``/api/health`` reponse contains multiple memory data points, including: 
...
The best way to monitor the status of the ``morphues-ui`` service is by utilizing the ``/ping`` enpoint on application nodes. This ensure the app is up and running. Monitoring the status of the ``morpheus-ui`` process/pid, or any other process, only informs if the process itself is running, not that the actual service successfully started and is running.

- ``https://appliance_url/ping`` returns **MORPHEUS PING** only when |morphues| is up and running. 

Checks monitoring morpheus availability should fail when ``https://appliance_url/ping`` does not return **MORPHEUS PING**

``https://appliance_node_ip/ping`` should be used by load balancer monitors/checks for HA configuraitons to monitor the status of morpheus on each app node.
