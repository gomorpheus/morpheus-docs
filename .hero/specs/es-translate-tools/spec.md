...
Translate all documentation in the `tools/` directory (~20 files) into Spanish using the AI translation pipeline.

## Changes

- `locale/es/LC_MESSAGES/tools/*.po` — Translated .po files

## Acceptance Criteria

1. All .po files for `tools/` have populated msgstr entries (no untranslated strings)
2. Translated .po files pass `msgfmt --check` validation
3. `make html` with language=es builds the tools section without errors or warnings
