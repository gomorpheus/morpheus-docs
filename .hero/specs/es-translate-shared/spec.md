...
Translate all documentation in the `shared/` directory (~2 files) into Spanish using the AI translation pipeline.

## Changes

- `locale/es/LC_MESSAGES/shared/*.po` — Translated .po files

## Acceptance Criteria

1. All .po files for `shared/` have populated msgstr entries (no untranslated strings)
2. Translated .po files pass `msgfmt --check` validation
3. `make html` with language=es builds the shared section without errors or warnings
