...
This page describes whole-device passthrough. NVIDIA vGPU slicing keeps the GPU bound to the NVIDIA Host driver and assigns programmed SR-IOV virtual functions instead of the full physical card. For that workflow, see :doc:`hvm/nvidia_vgpu`.
...
To create a new Service Plan, navigate to |AdmPla|. Click :guilabel:`+ Add` and then Service Plan. In the New Service Plan modal, first set the PROVISION TYPE configuration to "KVM." This will update the available configuration fields and reveal :guilabel:`GPU Count` and :guilabel:`GPU Type`. Select a physical GPU Type for whole-card passthrough. Indented Segment Types represent NVIDIA vGPU profiles and use the separate slicing workflow. See :doc:`/administration/plans_pricing/plans` for complete Service Plan guidance.
