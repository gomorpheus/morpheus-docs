...
The witness is a quorum-only member. It does not run VMs, mount the GFS2 datastore, or become an HVM compute Host. Deploy it outside the failure domain shared by the two HVM Hosts.
...
1. Create the two-Host HVM cluster using :doc:`building_clusters`. Do not add the GFS2 datastore yet.
...
4. Edit the HVM cluster, select the Distributed Worker in the **Witness** field, and save the cluster.
...
After adding the GFS2 datastore, open the cluster Quorum panel and confirm:

- Quorum status is **ACHIEVED**
- Two compute Hosts and one witness are configured
- Both Hosts are online and reachable
- The witness is reachable
- A Designated Coordinator is assigned
- GFS2 lockspaces are healthy and not waiting for fencing
...
- **Witness absent from the Quorum panel:** Confirm the Worker is selected on the cluster and a GFS2 datastore exists. Allow the cluster refresh to propagate updated quorum information.
...
- This topology is not a stretch cluster. It does not use site groups or the ``siteWitness`` designation.
